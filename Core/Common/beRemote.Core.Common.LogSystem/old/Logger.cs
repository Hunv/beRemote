...
namespace beRemote.Core.Common.LogSystem.old
{
    /// <summary>
    /// Provides log functionality for the application. Logger parameters are
    /// controlled via the appropiate properties. Logging is done using 
    ...
    static class Logger
    {
                ...
                if (logConsole != value)
                {
                    ...
                    if (null != consoleListener)
                    {
                        if (Trace.Listeners.Contains(consoleListener))
                        {
                            ...
                            {
                                Logger.Error("Error detaching ConsoleTraceListener from Trace: {0}",
                                    e.Message);
                            }
                        }
                    ...
                    {
                        Logger.Error("Error adding ConsoleTraceListener to Trace: " +
                            "{0}. Console logging will be disabled.", e.Message);
                        logConsole = false;
                        return;
                    }
                    Logger.Verbose("ConsoleTraceListener attached to Logger.");
                }
            }
        }
                ...
                if (logTextFile != cleanValue)
                {
                    ...
                    if (null != logFileListener)
                    {
                        if (Trace.Listeners.Contains(logFileListener))
                        {
                            ...
                            {
                                Logger.Error("Error detaching TextWriterTraceListener from Trace: {0}",
                                    e.Message);
                            }
                        }
                    ...
                    if (false == string.IsNullOrEmpty(logTextFile))
                    {
                        ...
                        if (false == Directory.Exists(directory))
                        {
                            ...
                            {
                                Logger.Error("Error creating parent directory for the " +
                        ...
                        //    catch (Exception e)
                        //    {
                        //        Logger.Error("Error cleaning up existing logfile at {0}:" +
                        ...
                        {
                            Logger.Error("Error adding TextWriterTraceListener to Trace: " +
                                "{0}. Textfile logging will be disabled.", e.Message);
                            logTextFile = null;
                            return;
                        }
                        Logger.Verbose("TextWriterTraceListener attached to Logger.");
                    }
                }
            }
        }
        ...
        private class CyclicTextWriterTraceListener : TextWriterTraceListener
        {
            ...
            private void CycleLogs()
            {
                ...
                {
                    Logger.Error("Error cycling logfiles: {0}.", e.Message);
                }
            }
